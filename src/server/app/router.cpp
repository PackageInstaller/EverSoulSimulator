...
namespace eversoul
{
    namespace
    {
        ...
        {
            return ini::get("account_profile", "responses") == "responses_newbie" ? AccountMode::Newbie : AccountMode::Full;
        }
        ...
        {
            ini::set("account_profile", mode == AccountMode::Newbie ? "responses_newbie" : "responses");
            return true;
        }
        ...
        const json::Value *userinfo_currency_array_const(const json::Value &root)
        {
            const json::Value *currency = root.find("currency");
            if (!currency || !currency->is_object())
                return nullptr;
            const json::Value *all = currency->find("allCurrency");
            return all && all->is_array() ? all : nullptr;
        }
        ...
        std::string fixture_currencies_json(const json::Value &root)
        {
            std::map<int, std::string> type_to_name;
            std::map<std::string, int> name_to_type;
            if (!read_currency_enum_maps(type_to_name, name_to_type))
                return "[]";
            std::string body = "[";
            bool first = true;
            const json::Value *all = userinfo_currency_array_const(root);
            if (all)
            {
                for (const auto &currency : all->arr)
                {
                    if (!currency.is_object())
                        continue;
                    const int type = fixture_currency_type(currency, name_to_type);
                    const json::Value *value = currency.find("value");
                    if (type <= 0)
                        continue;
                    if (!first)
                        body += ",";
                    body += "{\"type\":" + std::to_string(type) +
                            ",\"value\":" + std::to_string(value ? value->as_int64() : 0) + "}";
                    first = false;
                }
            }
            return body + "]";
        ...
        std::int64_t fixture_currency_value(const json::Value &root, int wanted_type)
        {
            std::map<int, std::string> type_to_name;
            std::map<std::string, int> name_to_type;
            if (!read_currency_enum_maps(type_to_name, name_to_type))
            ...
            for (const auto &currency : all->arr)
            {
                if (!currency.is_object() || fixture_currency_type(currency, name_to_type) != wanted_type)
                    continue;
                const json::Value *value = currency.find("value");
                return value ? value->as_int64() : 0;
            }
            return 0;
        }

        std::string fixture_heroes_json(const json::Value &root)
        {
            std::string body = "[";
            bool first = true;
            const json::Value *heroes = root.find("hero");
            if (heroes && heroes->is_array())
            {
                for (const auto &hero : heroes->arr)
                {
                    if (!hero.is_object())
                        continue;
                    const json::Value *idx = hero.find("idx");
                    const json::Value *hero_no = hero.find("heroNo");
                    if (!idx || !idx->is_string() || !hero_no)
                        continue;
                    const json::Value *level = hero.find("level");
                    const json::Value *grade_sno = hero.find("gradeSno");
                    const json::Value *race_sno = hero.find("raceSno");
                    if (!first)
                        body += ",";
                    body += "{\"idx\":\"" + json_escape(idx->str) + "\"" +
                            ",\"heroNo\":" + std::to_string(hero_no->as_int64()) +
                            ",\"level\":" + std::to_string(level ? level->as_int64() : 1) +
                            ",\"gradeSno\":" + std::to_string(grade_sno ? grade_sno->as_int64() : 0) +
                            ",\"raceSno\":" + std::to_string(race_sno ? race_sno->as_int64() : 0) + "}";
                    first = false;
                }
            }
            return body + "]";
        ...
        std::string fixture_items_json(const json::Value &root)
        {
            std::string body = "[";
            bool first = true;
            const json::Value *items = root.find("itemEtc");
            if (items && items->is_array())
            {
                for (const auto &item : items->arr)
                {
                    if (!item.is_object())
                        continue;
                    const json::Value *item_no = item.find("itemNo");
                    const json::Value *cnt = item.find("cnt");
                    if (!item_no || !cnt)
                        continue;
                    if (!first)
                        body += ",";
                    body += "{\"itemNo\":" + std::to_string(item_no->as_int64()) +
                            ",\"cnt\":" + std::to_string(cnt->as_int64()) + "}";
                    first = false;
                }
            }
            return body + "]";
        ...
        bool upsert_fixture_currency(json::Value &root, int type, std::int64_t value, std::string &reason)
        {
            std::map<int, std::string> type_to_name;
            std::map<std::string, int> name_to_type;
            if (!read_currency_enum_maps(type_to_name, name_to_type))
            {
                reason = "currency enum map not found";
                return false;
            }
            auto name_it = type_to_name.find(type);
            if (name_it == type_to_name.end())
            {
                reason = "unknown currency type";
                return false;
            }
            json::Value *all = userinfo_currency_array(root);
            for (auto &currency : all->arr)
            {
                if (!currency.is_object())
                    continue;
                if (fixture_currency_type(currency, name_to_type) == type)
                {
                    currency.set_string("type", name_it->second);
                    currency.set_string("value", std::to_string(value));
                    return true;
                }
            }
            json::Value next = json_object();
            next.set_string("type", name_it->second);
            next.set_string("value", std::to_string(value));
            all->arr.push_back(std::move(next));
            return true;
        }

        bool delete_fixture_currency(json::Value &root, int type)
        {
            std::map<int, std::string> type_to_name;
            std::map<std::string, int> name_to_type;
            if (!read_currency_enum_maps(type_to_name, name_to_type))
                return false;
            json::Value *all = userinfo_currency_array(root);
            const auto before = all->arr.size();
            all->arr.erase(std::remove_if(all->arr.begin(), all->arr.end(),
                [&](const json::Value &currency)
                {
                    return currency.is_object() && fixture_currency_type(currency, name_to_type) == type;
                }), all->arr.end());
            return all->arr.size() != before;
        }

        bool upsert_fixture_hero(json::Value &root, const std::string &idx, int hero_no,
                                 int level, int grade_sno, int race_sno, std::string &out_idx,
                                 std::string &reason)
        {
            if (hero_no <= 0)
            {
                reason = "invalid heroNo";
                return false;
            }
            out_idx = idx.empty() ? generated_hero_idx(hero_no) : idx;
            json::Value *heroes = json_array_member(root, "hero");
            for (auto &hero : heroes->arr)
            {
                if (!hero.is_object())
                    continue;
                const json::Value *hero_idx = hero.find("idx");
                if (hero_idx && hero_idx->is_string() && hero_idx->str == out_idx)
                {
                    hero.set_string("idx", out_idx);
                    hero.set_number("heroNo", hero_no);
                    hero.set_number("level", level > 0 ? level : 1);
                    hero.set_number("gradeSno", grade_sno);
                    hero.set_number("raceSno", race_sno);
                    return true;
                }
            }
            json::Value hero = json_object();
            hero.set_string("idx", out_idx);
            hero.set_number("heroNo", hero_no);
            hero.set_number("level", level > 0 ? level : 1);
            hero.set_number("gradeSno", grade_sno);
            hero.set_number("raceSno", race_sno);
            heroes->arr.push_back(std::move(hero));
            return true;
        }

        bool delete_fixture_hero(json::Value &root, const std::string &idx)
        {
            json::Value *heroes = json_array_member(root, "hero");
            const auto before = heroes->arr.size();
            heroes->arr.erase(std::remove_if(heroes->arr.begin(), heroes->arr.end(),
                [&](const json::Value &hero)
                {
                    const json::Value *hero_idx = hero.is_object() ? hero.find("idx") : nullptr;
                    return hero_idx && hero_idx->is_string() && hero_idx->str == idx;
                }), heroes->arr.end());
            return heroes->arr.size() != before;
        }

        bool upsert_fixture_item(json::Value &root, int item_no, int cnt, std::string &reason)
        {
            if (item_no <= 0)
            {
                reason = "invalid itemNo";
                return false;
            }
            json::Value *items = json_array_member(root, "itemEtc");
            for (auto &item : items->arr)
            {
                if (!item.is_object())
                    continue;
                const json::Value *no = item.find("itemNo");
                if (no && no->as_int64() == item_no)
                {
                    item.set_number("itemNo", item_no);
                    item.set_number("cnt", cnt);
                    return true;
                }
            }
        ...
        bool delete_fixture_item(json::Value &root, int item_no)
        {
            json::Value *items = json_array_member(root, "itemEtc");
            const auto before = items->arr.size();
            items->arr.erase(std::remove_if(items->arr.begin(), items->arr.end(),
                [&](const json::Value &item)
                {
                    const json::Value *no = item.is_object() ? item.find("itemNo") : nullptr;
                    return no && no->as_int64() == item_no;
                }), items->arr.end());
            return items->arr.size() != before;
        }
        ...
        std::string fixture_profile_userinfo_rel(const std::string &profile)
        {
            return profile + "/UserInfo.json";
        }

        bool read_userinfo_fixture_profile(const std::string &profile, json::Value &out, std::string &reason)
        {
            const std::string text = read_data_file(fixture_profile_userinfo_rel(profile));
            if (text.empty())
            {
                reason = "UserInfo fixture not found";
                return false;
        ...
        std::string fixture_account_json(const std::string &profile, bool active)
        {
            json::Value root;
            std::string reason;
            if (!read_userinfo_fixture_profile(profile, root, reason))
                return "";
            const json::Value *user = root.find("user");
            const json::Value *heroes = root.find("hero");
            std::string nickname = profile == "responses_newbie" ? "newbie" : "offline";
            std::string player_id = kDefaultPlayerId;
            std::int64_t created_at = 0;
            if (user && user->is_object())
            {
                if (const json::Value *nick = user->find("nickName"); nick && nick->is_string())
                    nickname = nick->str;
                if (const json::Value *idx = user->find("idx"); idx && idx->is_string())
                    player_id = idx->str;
                if (const json::Value *created = user->find("createdDt"))
                    created_at = created->as_int64();
            }
            const int hero_count = heroes && heroes->is_array() ? static_cast<int>(heroes->arr.size()) : 0;
            return "{\"id\":\"" + json_escape(profile) +
                   "\",\"nickname\":\"" + json_escape(nickname) +
                   "\",\"idp_code\":\"zd3\"" +
                   ",\"player_id\":\"" + json_escape(player_id) +
                   "\",\"profile_source\":\"" + json_escape(profile) +
                   "\",\"active\":" + (active ? "true" : "false") +
                   ",\"hero_count\":" + std::to_string(hero_count) +
                   ",\"created_at\":" + std::to_string(created_at) +
                   ",\"updated_at\":" + std::to_string(created_at) +
                   ",\"last_login\":0}";
        }
        ...
        HttpResponse handle_web_api([[maybe_unused]] uint64_t id, int fd, const HttpRequest &req)
        {
            ...
            if (path == "/web/api/status" && method == "GET")
            {
                ...
                std::string active_account = ini::get("account_profile", "responses");
            ...
            if (path.rfind("/web/api/accounts", 0) == 0)
            {
                ...
                {
                    const std::string active = selected_fixture_profile();
                    const std::vector<std::string> profiles = {"responses", "responses_newbie"};
                    std::string body = "[";
                    bool first = true;
                    for (const std::string &profile : profiles)
                    {
                        const std::string entry = fixture_account_json(profile, profile == active);
                        if (entry.empty())
                            continue;
                        if (!first)
                            body += ",";
                ...
                if (path.rfind(prefix, 0) == 0)
                {
                    ...
                    if (account_id != "responses" && account_id != "responses_newbie")
                    ...
                    {
                        const std::string entry = fixture_account_json(account_id, account_id == selected_fixture_profile());
                        if (entry.empty())
                            return HttpResponse{404, {{"Content-Type","application/json;charset=UTF-8"},{"Access-Control-Allow-Origin","*"}}, "{\"error\":\"not found\"}"};
                    ...
                    {
                        set_account_mode(account_id == "responses_newbie" ? AccountMode::Newbie : AccountMode::Full);
                        fixture_store().load(config().data_dir);
                        const bool ready = orm::ensure_ready(config().data_dir);
                        const std::string seed_profile = (account_id == "responses_newbie") ? "newbie" : "full";
                        const bool seeded = orm::reseed_from_profile(config().data_dir, seed_profile);
                        log_line(id, "MOCK", "fixture account selected " + account_id + " seed=" + seed_profile + (ready ? "" : " [ensure_ready FAILED]") + (seeded ? "" : " [reseed FAILED]"));
                        return json_ok("{\"ok\":true}");
                    }
                    ...
                    if (method == "PATCH" && action.empty())
                    {
                        json::Value root;
                        std::string reason;
                        if (!read_userinfo_fixture_profile(account_id, root, reason))
                            return json_ok("{\"ok\":false,\"reason\":\"" + json_escape(reason) + "\"}");
                        const std::string nick = body_json_string(req.body, "nickname", "");
                        const std::string pid  = body_json_string(req.body, "player_id", "");
                        json::Value *user = json_member(root, "user");
                        if (!nick.empty()) user->set_string("nickName", nick);
                        if (!pid.empty())  user->set_string("idx", pid);
                        if (!write_data_file(account_id + "/UserInfo.json", root.dump()))
                            return json_ok("{\"ok\":false,\"reason\":\"write failed\"}");
                        fixture_store().load(config().data_dir);
                        return json_ok("{\"ok\":true}");
                    }
                    ...
                    if (action_base == "currencies")
                    {
                        ...
                        if (method == "GET" && action_sub.empty())
                            return json_ok("{\"currencies\":" + fixture_currencies_json(root) + "}");
                        if (method == "GET" && !action_sub.empty())
                        {
                            const int type = std::atoi(action_sub.c_str());
                            const std::int64_t value = fixture_currency_value(root, type);
                            return json_ok("{\"type\":" + std::to_string(type) + ",\"value\":" + std::to_string(value) + "}");
                        }
                        if (method == "POST" && action_sub.empty())
                        {
                            ...
                            if (!upsert_fixture_currency(root, type, value, reason))
                                return json_ok("{\"ok\":false,\"reason\":\"" + json_escape(reason) + "\"}");
                            if (!write_data_file(account_id + "/UserInfo.json", root.dump()))
                                return json_ok("{\"ok\":false,\"reason\":\"write failed\"}");
                            fixture_store().load(config().data_dir);
                        ...
                        {
                            const int type = std::atoi(action_sub.c_str());
                            delete_fixture_currency(root, type);
                            if (!write_data_file(account_id + "/UserInfo.json", root.dump()))
                                return json_ok("{\"ok\":false,\"reason\":\"write failed\"}");
                            fixture_store().load(config().data_dir);
                            return json_ok("{\"ok\":true}");
                    ...
                    if (action_base == "heroes")
                    {
                        ...
                        if (!read_userinfo_fixture_profile(account_id, root, reason))
                            return json_ok("{\"ok\":false,\"reason\":\"" + json_escape(reason) + "\"}");
                        if (method == "GET" && action_sub.empty())
                            return json_ok("{\"heroes\":" + fixture_heroes_json(root) + "}");
                        if (method == "POST" && action_sub.empty())
                        {
                            ...
                            std::string out_idx;
                            if (!upsert_fixture_hero(root, idx, hero_no, level, grade_sno, race_sno, out_idx, reason))
                                return json_ok("{\"ok\":false,\"reason\":\"" + json_escape(reason) + "\"}");
                            if (!write_data_file(account_id + "/UserInfo.json", root.dump()))
                                return json_ok("{\"ok\":false,\"reason\":\"write failed\"}");
                            fixture_store().load(config().data_dir);
                        ...
                        if (method == "DELETE" && !action_sub.empty())
                        {
                            delete_fixture_hero(root, action_sub);
                            if (!write_data_file(account_id + "/UserInfo.json", root.dump()))
                                return json_ok("{\"ok\":false,\"reason\":\"write failed\"}");
                            fixture_store().load(config().data_dir);
                            return json_ok("{\"ok\":true}");
                    ...
                    if (action_base == "items")
                    {
                        ...
                        if (method == "DELETE" && !action_sub.empty())
                        {
                            ...
                            delete_fixture_item(root, item_no);
                            if (!write_data_file(account_id + "/UserInfo.json", root.dump()))
                                return json_ok("{\"ok\":false,\"reason\":\"write failed\"}");
                            fixture_store().load(config().data_dir);
                            return json_ok("{\"ok\":true}");
            ...
            if (path == "/web/api/setup/status" && method == "GET")
            {
                ...
                std::string account_profile = ini::get("account_profile", "responses");
