...
struct HeroEquipPresetSlotRow {
    int         preset_slot = 0;
    std::string preset_name;
};

struct HeroEquipPresetItemRow {
    int          preset_slot   = 0;
    int          hero_order    = 0;
    int          item_order    = 0;
    std::string  hero_idx;
...
struct ArbeitTaskRow {
    int          no                  = 0;
    int          date                = 0;
    int          arbeit_choice_no    = 0;
    int          arbeit_no           = 0;
    int          objet_no            = 0;
    std::int64_t start_timestamp     = 0;
    std::int64_t end_timestamp       = 0;
    int          state               = 0;
    std::string  hero_no_csv;
    std::string  kind;
    int          ingame_day          = 0;
};
...
public:
    ...
    void set_user_identity(const std::string& nick_name, const std::string& player_id);
    ...
    // ---- hero_equip_preset ----
    std::vector<HeroEquipPresetSlotRow> hero_equip_preset_slots() const;
    std::optional<HeroEquipPresetSlotRow> hero_equip_preset_slot(int preset_slot) const;
    std::vector<HeroEquipPresetItemRow> hero_equip_preset_items(int preset_slot) const;
    void upsert_hero_equip_preset_slot(const HeroEquipPresetSlotRow& row);
    void replace_hero_equip_preset_items(int preset_slot, const std::vector<HeroEquipPresetItemRow>& rows);
    ...
    // ---- arbeit_task ----
    std::vector<ArbeitTaskRow>    arbeit_tasks() const;
    std::optional<ArbeitTaskRow>  arbeit_task_by_no(int no) const;
    void                          upsert_arbeit_task(const ArbeitTaskRow& row);
...
private:
    ...
    ArbeitTaskRow     row_to_arbeit_task(sqlite3_stmt* st) const;
};
