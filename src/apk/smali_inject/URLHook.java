...
//     https:// , http://  ->  http://127.0.0.1:9991
//     wss://   , ws://    ->  ws://127.0.0.1:9991
// The downgrade is what lets the plaintext :9991 server work: HttpURLConnection
...
public final class URLHook {
    ...
    private static final String ACCOUNT_SELECT = "http://127.0.0.1:9991/account-select";
    private static final String PATCH_CDN_HOST = "patch.esoul.kakaogames.com";
    private static final String PATCH_CDN_LIVE_PATH = "/Live/";
    ...
    // *.game.kakao.com (*-zinny3). Scheme+host is rewritten to the local server,
    ...
    // IDP login hosts (Kakao web-view + web OAuth fallbacks for Google/Facebook/
    // Apple). Every login web-view is forced to the local account-select page so
    // that no real IDP is ever reached — the offline server owns account choice.
    private static final String[] LOGIN_HOSTS = {
        "kauth.kakao.com",
        "kapi.kakao.com",
        "accounts.kakao.com",
        "accounts.google.com",
    ...
    public static String redirect(String url) {
        ...
        try {
            if (isPatchCdnLiveUrl(url)) {
                return url;
            }

            // IDP login hosts take priority: collapse the whole OAuth web-view
            // (any scheme/path) to the local account-select page.
            for (String h : LOGIN_HOSTS) {
                if (url.contains(h)) {
    ...
    private static boolean isPatchCdnLiveUrl(String url) {
        return url.contains(PATCH_CDN_HOST) && url.contains(PATCH_CDN_LIVE_PATH);
    }
}
