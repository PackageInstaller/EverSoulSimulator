...
static const Table kTable = {
    ...
    { "injector.step_push_rev",      { {"ko","[5/6] SO 전송 + reverse tcp:9999/9998"},  {"en","[5/6] push SO + reverse tcp:9999/9998"},             {"zh","[5/6] SO 传输 + reverse tcp:9999/9998"} }},
    ...
    { "injector.step_push_rev2",     { {"ko","[2/4] SO 전송 + reverse tcp:9999/9998"},  {"en","[2/4] push SO + reverse tcp:9999/9998"},             {"zh","[2/4] SO 传输 + reverse tcp:9999/9998"} }},
    ...
    { "injector.push_ok",            { {"ko","  전송 완료: /data/local/tmp/libcawwyayy_patched.so"}, {"en","  pushed /data/local/tmp/libcawwyayy_patched.so"}, {"zh","  已传输：/data/local/tmp/libcawwyayy_patched.so"} }},
    { "injector.reverse_ok",         { {"ko","  reverse tcp:9999/9998 설정 완료"},       {"en","  reverse tcp:9999/9998 set"},                       {"zh","  reverse tcp:9999/9998 已设置"} }},
    ...
    { "injector.err_so",             { {"ko","ERROR: libcawwyayy_patched.so 없음: {path}"}, {"en","ERROR: libcawwyayy_patched.so not found: {path}"},     {"zh","ERROR: 找不到 libcawwyayy_patched.so：{path}"} }},
    ...
    { "injector.step_query_apk",     { {"ko","[2/8] 기기 APK 경로 조회"},                {"en","[2/8] query device APK paths"},                     {"zh","[2/8] 查询设备 APK 路径"} }},
    { "injector.step_pull_apk",      { {"ko","[3/8] 기기 APK 추출"},                     {"en","[3/8] pull device APKs"},                           {"zh","[3/8] 提取设备 APK"} }},
    { "injector.step_backup_apk",    { {"ko","[4/8] 로컬 백업"},                         {"en","[4/8] backup to local"},                            {"zh","[4/8] 本地备份"} }},
    { "injector.pull_apk_ok",        { {"ko","  추출 완료: {path}"},                     {"en","  pulled: {path}"},                                 {"zh","  已提取：{path}"} }},
    { "injector.backup_apk_ok",      { {"ko","  백업 완료: {path}"},                     {"en","  backed up: {path}"},                              {"zh","  已备份：{path}"} }},
    { "injector.err_pm_path",        { {"ko","ERROR: pm path 실패 (에버소울 미설치?)"},   {"en","ERROR: pm path failed (EverSoul not installed?)"},   {"zh","ERROR: pm path 失败（EverSoul 未安装？）"} }},
    { "injector.warn_pull_fail",     { {"ko","경고: APK pull 실패: {path}"},             {"en","WARNING: APK pull failed: {path}"},                 {"zh","警告：APK 提取失败：{path}"} }},
    { "injector.step_install2",      { {"ko","[5/8] APK 루트 직접 교체"},                {"en","[5/8] root replace APK"},                           {"zh","[5/8] Root 直替 APK"} }},
    { "injector.step_verify2",       { {"ko","[6/8] 설치 확인"},                         {"en","[6/8] verify install"},                             {"zh","[6/8] 验证安装"} }},
    { "injector.step_push_rev3",     { {"ko","[7/8] SO 전송 + reverse tcp:9999/9998"},   {"en","[7/8] push SO + reverse tcp:9999/9998"},             {"zh","[7/8] SO 传输 + reverse tcp:9999/9998"} }},
    { "injector.step_launch3",       { {"ko","[8/8] 게임 실행"},                         {"en","[8/8] launch game"},                                {"zh","[8/8] 启动游戏"} }},
    { "injector.push_apk_ok",       { {"ko","  전송: /data/local/tmp/base_patched.apk"},  {"en","  pushed /data/local/tmp/base_patched.apk"},         {"zh","  已传输：/data/local/tmp/base_patched.apk"} }},
    { "injector.root_replace_ok",   { {"ko","  교체 완료: {path}"},                       {"en","  replaced: {path}"},                                {"zh","  已替换：{path}"} }},
    { "injector.err_root_no_base",  { {"ko","ERROR: 기기 base.apk 경로 미확인 (pm path 실패?)"},  {"en","ERROR: device base.apk path unknown (pm path failed?)"},  {"zh","ERROR: 未确认设备 base.apk 路径（pm path 失败？）"} }},
    { "injector.err_root_replace",  { {"ko","ERROR: 루트 APK 교체 실패 (루트 권한 확인)"},         {"en","ERROR: root APK replace failed (check root access)"},     {"zh","ERROR: Root 替换 APK 失败（检查 root 权限）"} }},
    { "injector.push_swapper_ok",   { {"ko","  전송: /data/local/tmp/libswappywrapper.so"}, {"en","  pushed /data/local/tmp/libswappywrapper.so"},      {"zh","  已传输：/data/local/tmp/libswappywrapper.so"} }},
    { "injector.root_swapper_ok",   { {"ko","  교체 완료: lib/arm64/libswappywrapper.so"}, {"en","  replaced: lib/arm64/libswappywrapper.so"},          {"zh","  已替换：lib/arm64/libswappywrapper.so"} }},
};
