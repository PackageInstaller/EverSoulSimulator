...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        struct generated_always_t : basic_generated_always
        {
            ...
            generated_always_t(expression_type expression_, bool full, storage_type storage) : basic_generated_always{full, storage}, expression(std::move(expression_)) {}
...
namespace sqlite_orm { namespace internal {
    inline std::string wstr_to_utf8(const wchar_t* data, std::size_t len) {
        std::string r;
        r.reserve(len * 3);
#ifdef _WIN32
        for (std::size_t i = 0; i < len; ) {
            std::uint32_t cp;
            std::uint16_t w = static_cast<std::uint16_t>(data[i++]);
            if (w >= 0xD800u && w <= 0xDBFFu && i < len) {
                std::uint16_t lo = static_cast<std::uint16_t>(data[i]);
                if (lo >= 0xDC00u && lo <= 0xDFFFu) {
                    cp = 0x10000u + (static_cast<std::uint32_t>(w - 0xD800u) << 10) + (lo - 0xDC00u);
                    ++i;
                } else { cp = w; }
            } else { cp = w; }
            if (cp < 0x80u) { r.push_back(static_cast<char>(cp)); }
            else if (cp < 0x800u) { r.push_back(static_cast<char>(0xC0u|(cp>>6))); r.push_back(static_cast<char>(0x80u|(cp&0x3Fu))); }
            else if (cp < 0x10000u) { r.push_back(static_cast<char>(0xE0u|(cp>>12))); r.push_back(static_cast<char>(0x80u|((cp>>6)&0x3Fu))); r.push_back(static_cast<char>(0x80u|(cp&0x3Fu))); }
            else { r.push_back(static_cast<char>(0xF0u|(cp>>18))); r.push_back(static_cast<char>(0x80u|((cp>>12)&0x3Fu))); r.push_back(static_cast<char>(0x80u|((cp>>6)&0x3Fu))); r.push_back(static_cast<char>(0x80u|(cp&0x3Fu))); }
        }
#else
        for (std::size_t i = 0; i < len; ++i) {
            std::uint32_t cp = static_cast<std::uint32_t>(data[i]);
            if (cp < 0x80u) { r.push_back(static_cast<char>(cp)); }
            else if (cp < 0x800u) { r.push_back(static_cast<char>(0xC0u|(cp>>6))); r.push_back(static_cast<char>(0x80u|(cp&0x3Fu))); }
            else if (cp < 0x10000u) { r.push_back(static_cast<char>(0xE0u|(cp>>12))); r.push_back(static_cast<char>(0x80u|((cp>>6)&0x3Fu))); r.push_back(static_cast<char>(0x80u|(cp&0x3Fu))); }
            else { r.push_back(static_cast<char>(0xF0u|(cp>>18))); r.push_back(static_cast<char>(0x80u|((cp>>12)&0x3Fu))); r.push_back(static_cast<char>(0x80u|((cp>>6)&0x3Fu))); r.push_back(static_cast<char>(0x80u|(cp&0x3Fu))); }
    ...
    inline std::string wstr_to_utf8(const std::wstring& s) { return wstr_to_utf8(s.data(), s.size()); }
    inline std::wstring utf8_to_wstr(const char* data, std::size_t len) {
        std::wstring r;
        for (std::size_t i = 0; i < len; ) {
            std::uint32_t cp = 0;
            auto c = static_cast<unsigned char>(data[i]);
            std::size_t seqlen;
            if (c < 0x80u) { cp = c; seqlen = 1; }
            else if (c < 0xE0u) { cp = c & 0x1Fu; seqlen = 2; }
            else if (c < 0xF0u) { cp = c & 0x0Fu; seqlen = 3; }
            else { cp = c & 0x07u; seqlen = 4; }
            for (std::size_t j = 1; j < seqlen && i + j < len; ++j)
                cp = (cp << 6) | (static_cast<unsigned char>(data[i + j]) & 0x3Fu);
            i += seqlen;
#ifdef _WIN32
            if (cp < 0x10000u) { r.push_back(static_cast<wchar_t>(cp)); }
            else { cp -= 0x10000u; r.push_back(static_cast<wchar_t>(0xD800u|(cp>>10))); r.push_back(static_cast<wchar_t>(0xDC00u|(cp&0x3FFu))); }
#else
            r.push_back(static_cast<wchar_t>(cp));
#endif
        }
        return r;
    }
    inline std::wstring utf8_to_wstr(const std::string& s) { return utf8_to_wstr(s.data(), s.size()); }
} }
#endif
// #include "functional/cxx_optional.h"
...
namespace sqlite_orm
{
    ...
    struct field_printer<T, internal::match_if<std::is_base_of, std::wstring, T>>
    {
        ...
        {
            return internal::wstr_to_utf8(wideString);
        }
    };
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        struct dynamic_in_t : condition_t, in_base, negatable_t
        {
            ...
            dynamic_in_t(L left_, A argument_, bool negative_) : in_base{negative_}, left(std::move(left_)), argument(std::move(argument_)) {}
        };
        ...
        struct in_t : condition_t, in_base, negatable_t
        {
            ...
            in_t(L left_, decltype(argument) argument_, bool negative_) : in_base{negative_}, left(std::move(left_)), argument(std::move(argument_)) {}
        };
        ...
        struct order_by_base
        {
            ...
            order_by_base(decltype(asc_desc) asc_desc_, decltype(_collate_argument) _collate_argument_) : asc_desc(asc_desc_), _collate_argument(std::move(_collate_argument_)) {}
#endif
        };
        ...
        struct dynamic_order_by_entry_t : order_by_base
        {
            ...
            dynamic_order_by_entry_t(decltype(name) name_, int asc_desc_, std::string collate_argument_) : order_by_base{asc_desc_, std::move(collate_argument_)}, name(std::move(name_)) {}
        };
        ...
        struct like_t : condition_t, like_string, negatable_t
        {
            ...
            like_t(arg_t arg_, pattern_t pattern_, optional_container<escape_t> escape_) : arg(std::move(arg_)), pattern(std::move(pattern_)), arg3(std::move(escape_)) {}
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        struct highlight_t
        {
            ...
            highlight_t(argument0_type argument0, argument1_type argument1, argument2_type argument2) : argument0(std::move(argument0)), argument1(std::move(argument1)), argument2(std::move(argument2)) {}
        };
    }
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        {
#ifndef SQLITE_ORM_AGGREGATE_BASES_SUPPORTED
            column_t(std::string name, G memberPointer, S setter, std::tuple<Op...> op) : column_identifier{std::move(name)}, column_field<G, S>{memberPointer, setter},
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        struct union_t : public compound_operator<E...>, union_base
        {
            ...
            constexpr union_t(expressions_tuple compound, bool all) : compound_operator<E...>{std::move(compound)}, union_base{all} {}
        };
        ...
        struct common_table_expression
        {
            ...
            constexpr common_table_expression(explicit_colrefs_tuple explicitColumns, expression_type subselect) : explicitColumns{std::move(explicitColumns)}, subselect{std::move(subselect)}
            {
        ...
        struct with_t
        {
            ...
            with_t(bool recursiveIndicated, cte_type cte, expression_type expression) : recursiveIndicated{recursiveIndicated}, cte{std::move(cte)}, expression{std::move(expression)}
            {
...
namespace sqlite_orm
{
    ...
    public:
        ...
        pointer_binding(pointer_binding &&other) noexcept : p_{std::exchange(other.p_, nullptr)}, d_{std::move(other.d_)} {}
...
namespace sqlite_orm
{
                                                                   ...
                                                                   >::value>>
    {
        ...
        {
            auto stringData = this->string_data(value);
            std::string utf8Str = internal::wstr_to_utf8(stringData.first, static_cast<std::size_t>(stringData.second));
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        struct quoted_scalar_function
        {
            ...
            template <class... Args>
            consteval quoted_scalar_function(const char (&name)[N], Args &&...constructorArgs) : udf(std::forward<Args>(constructorArgs)...)
            {
        ...
        struct quoted_function_builder : cstring_literal<N>
        {
            ...
            [[nodiscard]] consteval auto quote(F callable) const
            {
                ...
                static_assert(std::is_member_function_pointer_v<decltype(static_cast<Sig F::*>(&F::operator()))>);
...
namespace sqlite_orm
{
    ...
    struct table_info
    {
        ...
        table_info(decltype(cid) cid_,
                   ...
                   decltype(pk) pk_) : cid(cid_), name(std::move(name_)), type(std::move(type_)), notnull(notnull_),
    ...
    struct table_xinfo
    {
        ...
        table_xinfo(decltype(cid) cid_,
                    ...
                    decltype(dflt_value) dflt_value_,
                    decltype(pk) pk_,
                    decltype(hidden) hidden_) : cid(cid_), name(std::move(name_)), type(std::move(type_)), notnull(notnull_),
...
#ifndef SQLITE_ORM_AGGREGATE_NSDMI_SUPPORTED
            indexed_column_t(column_type _column_or_expression) : column_or_expression(std::move(_column_or_expression)) {}
#endif
...
#ifndef SQLITE_ORM_AGGREGATE_BASES_SUPPORTED
            index_t(std::string name_, bool unique_, elements_type elements_) : index_base{std::move(name_), unique_}, elements(std::move(elements_)) {}
#endif
...
#ifndef SQLITE_ORM_AGGREGATE_BASES_SUPPORTED
            virtual_table_t(std::string name, module_details_type module_details) : basic_table{std::move(name)}, module_details{std::move(module_details)} {}
#endif
...
namespace sqlite_orm
{
    ...
    struct row_extractor<std::wstring, void>
    {
        std::wstring extract(const char *columnText) const
        {
            if (columnText)
            {
                return internal::utf8_to_wstr(columnText, std::char_traits<char>::length(columnText));
            }
            else
            {
                return {};
        ...
        std::wstring extract(sqlite3_stmt *stmt, int columnIndex) const
        {
            ...
            if (cStr)
            {
                return internal::utf8_to_wstr(cStr, std::char_traits<char>::length(cStr));
            }
            else
            {
                return {};
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
...
#ifndef SQLITE_ORM_AGGREGATE_NSDMI_SUPPORTED
            object_from_column_builder_base(sqlite3_stmt *stmt, int columnIndex = -1) : stmt{stmt}, columnIndex{columnIndex} {}
#endif
        };
        ...
        struct object_from_column_builder : object_from_column_builder_base
        {
            ...
            object_from_column_builder(object_type &object_, sqlite3_stmt *stmt_, int nextColumnIndex = 0) : object_from_column_builder_base{stmt_, nextColumnIndex - 1}, object(object_) {}
...
namespace sqlite_orm
{
    namespace internal
    {
        ...
        public:
            ...
            mapped_iterator(const db_objects_type &dbObjects, statement_finalizer stmt) : db_objects{&dbObjects}, stmt{std::move(stmt)}
            {
                this->step();
            }
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        struct prepared_statement_t : prepared_statement_base
        {
            ...
            prepared_statement_t(T expression_, sqlite3_stmt *stmt_, connection_ref con_) : prepared_statement_base{stmt_, std::move(con_)}, expression(std::move(expression_)) {}

            prepared_statement_t(prepared_statement_t &&prepared_stmt) : prepared_statement_base{prepared_stmt.stmt, std::move(prepared_stmt.con)},
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        struct mapped_view
        {
            ...
            mapped_view(storage_type &storage, connection_ref conn, Args &&...args) : storage(storage), connection(std::move(conn)), expression{{std::forward<Args>(args)...}} {}
...
namespace sqlite_orm::internal
{
    ...
    public:
        result_set_iterator(const db_objects_type &dbObjects, statement_finalizer stmt) : db_objects{dbObjects}, stmt{std::move(stmt)}
        {
            this->step();
        }
...
namespace sqlite_orm::internal
{
...
#endif
    {
        ...
        result_set_view(const db_objects_type &dbObjects, connection_ref conn, Select expression) : db_objects{dbObjects}, connection{std::move(conn)}, expression{std::move(expression)} {}
    ...
    namespace internal
    {
        ...
        struct transaction_guard_t
        {
            ...
            transaction_guard_t(connection_ref connection_,
                                ...
                                std::function<void()> rollback_func_) : connection(std::move(connection_)), commit_func(std::move(commit_func_)),
                                                                        rollback_func(std::move(rollback_func_)) {}

            transaction_guard_t(transaction_guard_t &&other) : commit_on_destroy(other.commit_on_destroy), connection(std::move(other.connection)),
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        struct backup_t
        {
            ...
            backup_t(backup_t &&other) : handle(std::exchange(other.handle, nullptr)), holder(std::move(other.holder)), to(other.to),
                                         from(other.from) {}
...
namespace sqlite_orm
{
    ...
    struct arg_values
    {
        ...
        struct iterator
        {
            ...
            iterator(const arg_values &container_, int index_) : container(container_), index(index_),
    ...
    namespace internal
    {
        ...
        struct udf_proxy
        {
            ...
            udf_proxy(std::string name,
                      ...
                      xdestroy_fn_t destroy,
                      sqlite_func_t func,
                      memory_space udfMemorySpace) : name{std::move(name)}, argumentsCount{argumentsCount}, constructAt{std::move(constructAt)},
            ...
            udf_proxy(std::string name,
                      ...
                      sqlite_func_t func,
                      final_call_fn_t finalAggregateCall,
                      memory_alloc udfAllocator) : name{std::move(name)}, argumentsCount{argumentsCount}, constructAt{std::move(constructAt)},
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        protected:
            storage_base(std::string filename, int foreignKeysCount) : pragma(std::bind(&storage_base::get_connection, this)),
...
namespace sqlite_orm
{
    namespace internal
    {
        ...
        struct partial_trigger_t
        {
            ...
            partial_trigger_t(T trigger_base, S... statements) : base{std::move(trigger_base)}, statements{std::make_tuple<S...>(std::forward<S>(statements)...)} {}
...
#ifndef SQLITE_ORM_AGGREGATE_BASES_SUPPORTED
            trigger_t(std::string name, T trigger_base, elements_type statements) : base_trigger{std::move(name)}, base(std::move(trigger_base)), elements(std::move(statements)) {}
#endif
        };
        ...
        struct trigger_update_type_t : trigger_type_base_t
        {
            ...
            trigger_update_type_t(trigger_timing timing, trigger_type type, Cs... columns) : trigger_type_base_t(timing, type), columns(std::make_tuple<Cs...>(std::forward<Cs>(columns)...)) {}
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        private:
            ...
            {
                return quote_string_literal(internal::wstr_to_utf8(c, std::char_traits<wchar_t>::length(c)));
            }
#endif
#ifdef SQLITE_ORM_STRING_VIEW_SUPPORTED
            ...
            {
                return quote_string_literal(internal::wstr_to_utf8(c.data(), c.size()));
            }
#endif
#endif
            /**
...
namespace sqlite_orm
{
    namespace internal
    {
        ...
        static auto make_cte_column(std::string name, const ColRef &finalColRef)
        {
            static_assert(std::is_class_v<table_type_of_t<ColRef>>);
...
namespace sqlite_orm
{
    ...
    namespace internal
    {
        ...
        struct storage_t : storage_base
        {
             ...
             */
            storage_t(std::string filename, db_objects_type dbObjects) : storage_base{std::move(filename), foreign_keys_count(dbObjects)}, db_objects{std::move(dbObjects)} {}
